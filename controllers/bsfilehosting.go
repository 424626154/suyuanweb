...
// 图床服务器
type BsFileHostingController struct {
	beego.Controller
}

func (c *BsFileHostingController) Get() {
	beego.Debug("BsFileHostingController Get")
...
// post上传 返回图片地址
func (c *BsFileHostingController) Post() {

	// token := c.Input().Get("token")
	// beego.Debug("c:", c)
	beego.Debug("BsFileHostingController post")
	image_name := ""
	...
	if fh != nil {
		...
		err = c.SaveToFile(file_patn, path.Join("bsfilehosting", image_name))
		if err != nil {
			beego.Error(err)
		}
	}
